    ...
    public partial class MainWindow : Window
    {
        ...
        private void saveChanges_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                // Insert messagebox so the user knows that the changes are saved to the database
                MessageBox.Show("Changes saved to Database");

                // Disable Save button till changes are made
                saveChanges.IsEnabled = false;
            }
            // Exercise 3: Task 3a: If an OptimisticConcurrencyException occurs then another user has changed the same students earlier, 
            // then overwrite their changes with the new data
            catch (OptimisticConcurrencyException)
            {
                // Overwrite changes with new data
                this.schoolContext.Refresh(RefreshMode.ClientWins, schoolContext.Students);
                this.schoolContext.SaveChanges();
            }
            // Exercise 3: Task 3b: If an UpdateException occurs then report the error to the user and rollback
            catch (UpdateException updEx)
            {
                // Display the reason for the exception and perform a Rollback
                MessageBox.Show(updEx.InnerException.Message, "Error saving changes");
                this.schoolContext.Refresh(RefreshMode.StoreWins, schoolContext.Students);
            }
            // Exercise 3: Task 3c: If some other sort of error has occurs, report the error to the user and retain the data so the user can try again
